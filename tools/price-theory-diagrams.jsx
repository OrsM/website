...
// Coordinate system: Q on x (0-100), P on y (0-100)
// Canvas mapping
...
function SupplyDemand() {
  ...
  // Linear S&D: D: P = 90 - Q + dShift, S: P = 10 + Q + sShift
  ...
  // Equilibrium: 90 - Q + dShift = 10 + Q + sShift + taxAmt → Q = (80 + dShift - sShift - taxAmt)/2
  ...
  const sPoints = Array.from({ length: 50 }, (_, i) => {
    const q = (i / 49) * 95;
    return [q, supplyP(q)];
  }).filter(([_, p]) => p > 0 && p < 100);

  const sPointsNoTax = Array.from({ length: 50 }, (_, i) => {
    const q = (i / 49) * 95;
    return [q, supplyPNoTax(q)];
  }).filter(([_, p]) => p > 0 && p < 100);
  ...
  return (
    <div>
      <canvas ref={ref} style={{ width: "100%", height: 320, display: "block", marginBottom: 16 }} />
      <div style={{ fontSize: 13, color: "#777", marginBottom: 12, fontStyle: "italic", textAlign: "center" }}>
        Equilibrium: Q* = {eqQ.toFixed(0)}, P* = {eqP.toFixed(0)}
        {taxAmt > 0 && ` · Tax wedge = ${(eqP - sellerP).toFixed(0)}`}
      </div>
      <Slider label="Shift Demand" value={dShift} onChange={setDShift} min={-40} max={40} color="#c9553a" />
      <Slider label="Shift Supply" value={sShift} onChange={setSShift} min={-40} max={40} color="#4a9e6e" />
      <Slider label="Tax Amount" value={taxAmt} onChange={setTaxAmt} min={0} max={40} color="#8b6914" />
    </div>
  );
}
...
function CostCurves() {
  ...
  // MC = mcBase + 0.4*Q (U-shaped approximation)
  // TC = FC + mcBase*Q + 0.2*Q²
  // AC = TC/Q, AVC = (TC-FC)/Q
  ...
  // Min AC
  // AC = FC/Q + mcBase + 0.2Q, d(AC)/dQ = -FC/Q² + 0.2 = 0 → Q = sqrt(FC/0.2)
  ...
  const ref = useCanvas((ctx, w, h) => {
    ...
    const labelAt = (pts, text, color) => {
      ...
      drawLabel(ctx, pt[0], pt[1], w, h, PAD, text, color, 4, -2);
    };
  ...
  return (
    <div>
      ...
      <Slider label="Fixed Cost" value={fc} onChange={setFc} min={50} max={600} color="#4a6fa5" />
      <Slider label="MC Base" value={mcBase} onChange={setMcBase} min={2} max={25} color="#c9553a" />
    </div>
  );
}
...
function MonopolyDiagram() {
  ...
  const dPoints = Array.from({ length: 50 }, (_, i) => {
    const q = (i / 49) * 100;
    return [scQ(q), scP(demand(q))];
  }).filter(([, p]) => p > 0);

  const mrPoints = Array.from({ length: 50 }, (_, i) => {
    const q = (i / 49) * 100;
    return [scQ(q), scP(mr(q))];
  }).filter(([, p]) => p > 0);
  ...
  const ref = useCanvas((ctx, w, h) => {
    ...
    const dwlPts = [
      [scQ(qMon), scP(pMon)],
      [scQ(qComp), scP(mcLevel)],
      [scQ(qMon), scP(mcLevel)],
    ];
    fillArea(ctx, dwlPts, w, h, PAD, "rgba(201, 85, 58, 0.15)");
    ...
    fillArea(ctx, profitPts, w, h, PAD, "rgba(74, 111, 165, 0.1)");
    ...
    // DWL label
    const dwlCx = scQ((qMon + qComp) / 2);
    const dwlCy = scP((pMon + mcLevel) / 2.5);
    ctx.fillStyle = "#c9553a";
    ctx.font = "bold 12px Georgia, serif";
    ctx.textAlign = "center";
    const [dx, dy] = toCanvas(dwlCx, dwlCy, w, h, PAD);
  ...
  return (
    <div>
      <canvas ref={ref} style={{ width: "100%", height: 320, display: "block", marginBottom: 16 }} />
      <div style={{ fontSize: 13, color: "#777", marginBottom: 12, fontStyle: "italic", textAlign: "center" }}>
        Monopoly: Qm={qMon.toFixed(0)}, Pm={pMon.toFixed(0)} · Lerner = {lerner} · Competitive: Qc={qComp.toFixed(0)}
      </div>
      <Slider label="Demand Intercept" value={demIntercept} onChange={setDemIntercept} min={40} max={100} color="#c9553a" />
      <Slider label="Marginal Cost" value={mcLevel} onChange={setMcLevel} min={5} max={50} color="#4a9e6e" />
    </div>
  );
}
...
function SurplusDiagram() {
  ...
  // At effective price: Qd = 85 - P, Qs = P - 15
  ...
  const ref = useCanvas((ctx, w, h) => {
    ...
    const csPoints = [
      [scQ(0), scP(85)],
      [scQ(transactedQ), scP(demand(transactedQ))],
      [scQ(transactedQ), scP(effectiveP)],
      [scQ(0), scP(effectiveP)],
    ];
    fillArea(ctx, csPoints, w, h, PAD, "rgba(74, 111, 165, 0.2)");

    // Producer surplus
    const psPoints = [
      [scQ(0), scP(effectiveP)],
      [scQ(transactedQ), scP(effectiveP)],
      [scQ(transactedQ), scP(supply(transactedQ))],
      [scQ(0), scP(15)],
    ];
    fillArea(ctx, psPoints, w, h, PAD, "rgba(74, 158, 110, 0.2)");

    // DWL if price control
    if (isFloor || isCeiling) {
      ...
      fillArea(ctx, dwlPoints, w, h, PAD, "rgba(201, 85, 58, 0.2)");
    ...
    // Price control line
    if (isFloor || isCeiling) {
      drawLine(ctx, [[0, scP(effectiveP)], [95, scP(effectiveP)]], w, h, PAD, "#8b6914", 2, [8, 4]);
      const label = isFloor ? "Price Floor" : "Price Ceiling";
      ctx.fillStyle = "#8b6914";
      ctx.font = "12px Georgia, serif";
      ctx.textAlign = "right";
      const [, py] = toCanvas(90, scP(effectiveP), w, h, PAD);
      ctx.fillText(label, w - PAD - 4, py - 6);
    }
    ...
    // Surplus values
    const cs = 0.5 * transactedQ * (demand(0) - effectiveP);
    const ps = 0.5 * transactedQ * (effectiveP - supply(0));
  ...
  return (
    <div>
      ...
      <Slider label="Price Floor" value={pFloor} onChange={(v) => { setPFloor(v); if (v > 0) setPCeiling(0); }} min={0} max={80} color="#8b6914" />
      <Slider label="Price Ceiling" value={pCeiling} onChange={(v) => { setPCeiling(v); if (v > 0) setPFloor(0); }} min={0} max={80} color="#7c3f58" />
    </div>
  );
}

function Slider({ label, value, onChange, min, max, color }) {
  return (
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 3, color: "#999", marginBottom: 2 }}>
          McCloskey · Applied Theory of Price
        </div>
        <h1 style={{ fontSize: 24, fontWeight: 400, margin: 0 }}>Interactive Diagrams</h1>
      </div>

      <div style={{ display: "flex", gap: 0, marginBottom: 20, borderBottom: "1px solid #e0dcd6" }}>
        {TABS.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      <div style={{
        ...
        {tab === "sd" && "Shift demand or supply to see how equilibrium moves. Add a tax to see the wedge between buyer and seller prices. Notice: the more inelastic side bears more of the tax burden."}
        {tab === "cost" && "Watch how MC always crosses AC at its minimum — like a new test score pulling your average up or down. Raising fixed costs shifts AC up but doesn't touch MC. The firm produces where P = MC."}
        {tab === "monopoly" && "The monopolist restricts output to where MR = MC, charging above MC. The gap between Qm and Qc is the deadweight loss — trades that would benefit both buyer and seller but don't happen. The Lerner index measures market power."}
        {tab === "surplus" && "Consumer surplus is willingness-to-pay minus price paid. Set a price floor (minimum wage, agricultural supports) or ceiling (rent control) to see how the surplus shrinks and deadweight loss appears."}
      </div>
    </div>
  );
}
